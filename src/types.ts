...
export interface Check<T = unknown> {
	...
	): CheckResult | Diagnostic | Diagnostic[] | null | undefined
}
...
export type SpecNode<T> =
	T extends NodeSpec<infer TNode, infer TOptional>
...
export interface ListNode<TItemNode extends AnyNode = AnyNode>
	extends BaseNode<NodeValue<TItemNode>[]> {
	readonly kind: 'list'
	readonly items: ComputedRef<TItemNode[]>
	byKey(key: PropertyKey): TItemNode | undefined
}

type RecordDynamicProps<TItemNode extends AnyNode, TKey extends string> = [
	TKey,
] extends [never]
	? object
...
export type TreeNode<TChildren extends SectionChildren> =
	SectionNode<TChildren> & {
