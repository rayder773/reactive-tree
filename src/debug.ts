...
export interface DebugStore {
	...
	registerNode(
		node: AnyNode,
		info: Omit<DebugNodeInfo, 'active'> & { active?: boolean },
	): void
	...
	runWithReader<T>(
		input: { readerId: string; reason: DependencyReason },
		fn: () => T,
	): T
...
function edgeKey(
	edge: Pick<DependencyEdge, 'readerId' | 'targetId' | 'targetProp' | 'reason'>,
): string {
...
export function createDebugStore(): DebugStore {
	...
	const store: DebugStore = {
		...
		crossEdges: computed(() =>
			Array.from(edgesByKey.values()).filter(
				(edge) => !nodesById.has(edge.targetId),
			),
		),
		...
		runWithReader(input, fn) {
			if (input.reason === 'watch') {
				...
				try {
					...
					for (const [key, edge] of Array.from(toRaw(edgesByKey).entries())) {
						if (
							edge.readerId === input.readerId &&
							edge.createdAtRunId !== nextRunId
						) {
							edgesByKey.delete(key)
						}
					}
			...
			const wrap = (value: unknown): unknown => {
				...
				const proxy = new Proxy(value, {
					get(target, property, receiver) {
						...
						if (
							isTrackableNode(child) &&
							conditionalNodeIds.has(child.__debug.id)
						) {
							store.trackRead({
