...
describe('reactive tree core', () => {
	...
	it('withWatch can read a later sibling during tree creation', () => {
		const tree = createTree({
			watched: withWatch(state('initial'), [(self) => self.source.value]),
			source: state('ready'),
		})
	...
	it('debug marks disappeared when node inactive', () => {
		...
		expect(
			tree.debug.getNodes().find((node) => node.id === 'details')?.active,
		).toBe(true)
		tree.enabled.set(false)
		expect(tree.details).toBeUndefined()
		expect(
	...
	it('debug does not duplicate edges across reruns', () => {
		...
		const edges = tree.debug
			.getEdges()
			.filter(
				(edge) => edge.readerId === 'derived' && edge.targetId === 'source',
			)
