...
export function min(limit: number, options: { mode?: CheckMode } = {}) {
  return check<number>(
    value => (value < limit ? error('min', `Value must be at least ${limit}`, { min: limit }) : undefined),
...
export function max(limit: number, options: { mode?: CheckMode } = {}) {
  return check<number>(
    value => (value > limit ? error('max', `Value must be at most ${limit}`, { max: limit }) : undefined),
