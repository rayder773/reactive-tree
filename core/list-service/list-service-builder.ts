import type { MappedListContract, RuntimeKey } from '../mapped-list/mapped-list.types'
import type { AbortService } from '../services/AbortService'
import type { FiltersServiceContract, FiltersServiceOptions } from '../services/filters/filters-service.types'
import type { LoadingService, LoadingStatus } from '../services/LoadingService'
import type { PaginationService, PaginationState } from '../services/PaginationService'
import type { SortingServiceContract, SortingServiceOptions, SortingState } from '../services/sorting/sorting-service.types'
import type { AppRuntime } from '../runtime/AppRuntime'
import type { EntityListContract } from '../mapped-list/mapped-list.types'
...
interface BuilderConfig<TEntity, TId extends string> {
	...
	serverQueryFn: ((params: Record<string, unknown>, signal: AbortSignal) => Promise<unknown>) | undefined
	mapResponse: ((response: unknown) => { items: readonly TEntity[]; total: number }) | undefined
	byIdsFn: ((ids: readonly TId[], signal: AbortSignal) => Promise<readonly TEntity[]>) | undefined
}
...
export class ListServiceBuilder<TEntity, TId extends string = string> {
	...
	withLoading(): this {
		return this.clone({ loadingService: this.config.app.createLoadingService() })
	}
	...
	withServerSorting<TField extends string>(options: SortingServiceOptions<TField>): this {
		return this.clone({
			sortingService: this.config.app.createSortingService(options) as SortingServiceContract<string>,
			isSortingServer: true,
		})
	}

	withClientSorting<TField extends string>(options: SortingServiceOptions<TField>): this {
		return this.clone({
			sortingService: this.config.app.createSortingService(options) as SortingServiceContract<string>,
	...
	withServerFilters<TFilters extends Record<string, unknown>>(options: FiltersServiceOptions<TFilters>): this {
		return this.clone({
			filtersService: this.config.app.createFiltersService(options) as FiltersServiceContract<Record<string, unknown>>,
			isFiltersServer: true,
		})
	}

	withClientFilters<TFilters extends Record<string, unknown>>(options: FiltersServiceOptions<TFilters>): this {
		return this.clone({
			filtersService: this.config.app.createFiltersService(options) as FiltersServiceContract<Record<string, unknown>>,
	...
	withServerQuery<TRawResponse>(
		fn: (params: Record<string, unknown>, signal: AbortSignal) => Promise<TRawResponse>,
		options: { mapResponse: (response: TRawResponse) => { items: readonly TEntity[]; total: number } },
	): this {
		return this.clone({
			serverQueryFn: fn as BuilderConfig<TEntity, TId>['serverQueryFn'],
			mapResponse: options.mapResponse as BuilderConfig<TEntity, TId>['mapResponse'],
		})
	}

	withByIdsQuery(fn: (ids: readonly TId[], signal: AbortSignal) => Promise<readonly TEntity[]>): this {
	...
	private clone(overrides: Partial<BuilderConfig<TEntity, TId>>): this {
		return new ListServiceBuilder<TEntity, TId>({ ...this.config, ...overrides }) as this
	}
}

class ListServiceImpl<TEntity, TId extends string = string> implements ListService<TEntity, TId> {
	...
	async getByIds(ids: readonly TId[], _key?: RuntimeKey): Promise<readonly TEntity[]> {
		const { byIdsFn, abortService, loadingService, name } = this.config

		if (byIdsFn === undefined) {
			throw new Error('ListService: byIdsQuery is not configured. Call withByIdsQuery() in the builder.')
		}

		const missingIds = ids.filter((id) => !this.has(id))

		if (missingIds.length === 0) {
			return ids.map((id) => this.get(id)).filter((e): e is TEntity => e !== undefined)
		}
		...
		if (loadingService !== undefined) {
			await Promise.all(
				missingIds.map((id) => loadingService.run(() => batchPromise, [name, 'entity', id])),
			)
		} else {
			await batchPromise
		}
	...
	sorting(key?: RuntimeKey): SortingState<string> {
		...
		if (sortingService === undefined) {
			throw new Error('ListService: sorting is not configured. Call withServerSorting() or withClientSorting() in the builder.')
		}
	...
	filters(key?: RuntimeKey): Record<string, unknown> {
		...
		if (filtersService === undefined) {
			throw new Error('ListService: filters is not configured. Call withServerFilters() or withClientFilters() in the builder.')
		}
	...
	pagination(key?: RuntimeKey): PaginationState {
		...
		if (paginationService === undefined) {
			throw new Error('ListService: pagination is not configured. Call withServerPagination() or withClientPagination() in the builder.')
		}
	...
	loading(key?: RuntimeKey): LoadingStatus {
		...
		if (loadingService === undefined) {
			throw new Error('ListService: loading is not configured. Call withLoading() in the builder.')
		}

		return loadingService.get(key)
	}

	private async executeLoad(key: RuntimeKey | undefined, mode: 'replace' | 'append'): Promise<readonly TEntity[]> {
		const {
			queryFn,
			serverQueryFn,
			mapResponse,
		...
		if (queryFn === undefined && serverQueryFn === undefined) {
			throw new Error('ListService: query is not configured. Call withQuery() or withServerQuery() in the builder.')
		}

		const list = mappedList.list(key)

		const doRequest = async (signal: AbortSignal): Promise<readonly TEntity[]> => {
