...
export function asyncNode<T, TInput = void>(
	options: AsyncNodeOptions<T, TInput> = {},
	...
	return {
		build(context: BuildContext): AsyncNode<T, TInput> {
			...
			let fetcher: ((input: TInput, signal: AbortSignal) => Promise<T>) | null =
				null
